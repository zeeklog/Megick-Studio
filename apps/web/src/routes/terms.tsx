...
export const Route = createFileRoute("/terms")({
  head: () => {
    ...
    return seoHead({
      title: `${translate(locale, "terms.title")} - Megick`,
      description: translate(locale, "terms.intro"),
      path: "/terms",
      locale,
    });
  },
      ...
      <div className="relative overflow-hidden pb-14 pt-16 sm:pb-16 sm:pt-24">
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 space-y-10 sm:mt-12 sm:space-y-12">
            <p className="text-base leading-relaxed text-muted-foreground sm:text-lg">{t("terms.intro")}</p>

            {sections.map((num) => {
              const titleKey = `terms.section${num}.title` as TranslationKey;
              const contentKey = `terms.section${num}.content` as TranslationKey;
              const itemsKey = `terms.section${num}.items` as TranslationKey;
